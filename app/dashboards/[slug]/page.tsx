...
export default async function DashboardSlugPage({ params }: { params: { slug: string } }) {
  ...
  // hard guard: if cookie is absent, ensure we never pass canEdit=true
  const canEdit = Boolean(isEdit);

  return (
    <main>
      {canEdit ? <EditBanner /> : null}
      <NavBar title={slug.toUpperCase()} initialActive={canEdit} />
      ...
      <Card>
        ...
        <CardContent>
          <DashboardManager slug={slug} initialLinks={links.items} initialWidgets={widgets.items} serviceEmail={serviceEmail} canEdit={canEdit} />
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          {widgets.items.length === 0 ? (
            <div className="text-sm text-gray-500">Нет виджетов, {isEdit ? 'создайте первый' : 'перейдите в Edit dashboard, чтобы добавить'}</div>
          ) : (
